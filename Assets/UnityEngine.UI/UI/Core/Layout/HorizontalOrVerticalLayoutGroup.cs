    ...
    public abstract class HorizontalOrVerticalLayoutGroup : LayoutGroup
    {
        ...
        protected void CalcAlongAxis(int axis, bool isVertical)
        {
            ...
            for (int i = 0; i < rectChildren.Count; i++)
            {
                ...
                float min = LayoutUtility.GetMinSize(child, axis);//0
                float preferred = LayoutUtility.GetPreferredSize(child, axis);//sliced uisprite return 10
                float flexible = LayoutUtility.GetFlexibleSize(child, axis);//-1
        ...
        protected void SetChildrenAlongAxis(int axis, bool isVertical)
        {
            ...
            bool alongOtherAxis = (isVertical ^ (axis == 1));//异或 可以相当于 ！=（不等于）来替换
            if (alongOtherAxis)
            {
